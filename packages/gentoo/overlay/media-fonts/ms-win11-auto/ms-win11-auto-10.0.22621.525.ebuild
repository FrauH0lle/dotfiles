...
pkg_setup() {
	elog "- Examining locally available fonts"
	for _font in ${_ttf_ms_win11[@]}; do
		_allFiles+=( $_font )
	...
	if [ ${#_missingFonts[@]} -gt 0 ]; then
		elog "- Fonts are missing"
		einfo "- Mount filesystems as a non-privileged user: "
		touch test.mount
		...
		if [ $_unprivilegedMountAllowed ]; then
			...
			if ! grep -qs $_loopDev /proc/mounts; then
				elog "  - Mounting loop device: $_loopDev"
				udisksctl mount -t udf -b "$_loopDev" --no-user-interaction
			fi
			_mountpoint=$(findmnt -nfr -o target -S $_loopDev)
			elog "  - Loop device mounted as ISO at: $_mountpoint"

			elog "  - Extracting files from online Windows installation image"
			7z e -aoa "${_mountpoint}/sources/install.wim" \
			   Windows/{Fonts/"*".{ttf,ttc},System32/Licenses/neutral/"*"/"*"/license.rtf}

			elog "  - Unmounting loop device $_loopDev as ISO at: $_mountpoint"
			udisksctl unmount -b "$_loopDev" --no-user-interaction
			elog "  - Deleting loop device: $_loopDev"
			udisksctl loop-delete -b "$_loopDev" --no-user-interaction
			elog "  - Unmounting HTTP file"
			fusermount -uz mnt/http
			rmdir -p mnt/http
		else
			einfo "not allowed"
			elog "- Preparing download of full ISO"
			elog "  - Checking free disk space required for download and extraction"
			_freeDiskSpace=$(($(stat -f --format="%a*%S" .)))
			_downloadSize=$(curl -sIL "$_iso" | grep -i Content-Length | tail -n1 | awk '{print $2}' | tr -d '\r\n')
			[ -z $_downloadSize ] && elog "Unable to determine file size of:\n${_iso}\n" && exit 255
			_requiredDiskSpace=$((${_downloadSize}*2))
			if [ $_requiredDiskSpace -gt $_freeDiskSpace ]; then
				elog "Not enough free disk space"
				elog "Needed: $((${_requiredDiskSpace}/1048576)) MiB"
				elog "Available: $((${_freeDiskSpace}/1048576)) MiB"
				exit 255
			fi

			elog "  - Downloading ISO"
			curl -JLO "$_iso"
			elog "  - Extracting Windows installation image"
			7z x $(echo "$_iso" | awk -F "/" '{print $NF}') sources/install.wim

			elog "  - Extracting files from local Windows installation image"
			7z e sources/install.wim \
			   Windows/{Fonts/"*".{ttf,ttc},System32/Licenses/neutral/"*"/"*"/license.rtf}

			elog "  - Cleaning up temporary files"
			rm $(echo "$_iso" | awk -F "/" '{print $NF}')
			rm -rf sources
		fi
	fi

	elog "- Verifying file integrity"
	...
	if [ $_fail ]; then
		eerror "One or more files fail the integrity check or are missing. Exiting."
		exit 255
	else
		for _i in ${!_allFiles[@]}; do
			mv -v "${_allFiles[$_i]}" "${WORKDIR}/${_allFiles[$_i]}"
		done
	fi
}

src_install() {
	install -Dm644 "${_ttf_ms_win11[@]}" -t "${D}/usr/share/fonts/TTF"
	install -Dm644 license.rtf -t "${D}/usr/share/licenses/$PN"
}
